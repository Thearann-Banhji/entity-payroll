...
module.exports.gets = async (event, context) => {
  ...
  const strQuery = event.multiValueQueryStringParameters
  let search = ''
  if (strQuery) {
    if (Object.prototype.hasOwnProperty.call(strQuery, 'filter[filters][0][value]')) {
      const strSearch = strQuery['filter[filters][0][value]']
      search = strSearch[0]
    }
  }
  let params = ''
